...
gtf_file_name <- basename(gtf_download_link)
gtf_file_name

whether_gtf_downloaded <- file.exists(gtf_file_name)
whether_gtf_downloaded
```
...
```{r read-input-file}
if (whether_gtf_downloaded == FALSE) {
    download.file(url = gtf_download_link,
                  destfile = gtf_file_name,
                  method = "internal")
} else {
    cat(gtf_file_name, "has already been downloaded to the working directory", sep = " ", "\n")
}
```

```{r}
path_to_input_file <- gtf_file_name
