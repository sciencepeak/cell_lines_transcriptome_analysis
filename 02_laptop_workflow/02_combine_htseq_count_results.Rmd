...
```{r}
whether_compressed_htseq_count <- TRUE

# All the downloaded HTSeq-count result text files should be put under one directory.
original_htseq_count_result_directory <- file.path(".", "htseq_count_result_directory")

temporary_htseq_count_result_directory <-
    file.path(getwd(), "temporary_htseq_count_result_directory") %T>%
    dir.create(., showWarnings = F, recursive = T)

# In case that previous run result are retained.
list.files(temporary_htseq_count_result_directory, full.names = T) %>%
    unlink
```
...
output_csv_file_name <- "htseq_count_expression_matrix.csv"
```

# Prepare the htseq result files.

Copy the compressed files to a temporary folder and decompress them. The raw compressed files will be deleted automatically.

```{r}
file.copy(from = list.files(original_htseq_count_result_directory, full.names = T),
          to = temporary_htseq_count_result_directory)
if (whether_compressed_htseq_count == TRUE) {
    list.files(temporary_htseq_count_result_directory, full.names = T) %>%
        sapply(., bunzip2)
}
```
...
paths_to_htseq_count_files <- list.files(temporary_htseq_count_result_directory, full.names = T)
...
sample_names <- list.files(temporary_htseq_count_result_directory) %>%
...
unlink(temporary_htseq_count_result_directory, recursive = T)
